...
DEFAULT_STORAGE_STATE_PATH = os.getenv(
    "BAITOOLS_STORAGE_STATE_PATH",
    str(Path(__file__).resolve().parents[3] / ".playwright" / "baitools_storage_state.json"),
)


class BAItoolsAuthStrategy(BaseAuthStrategy):
    ...
    async def ensure_authenticated(self, browser):
        if not self.session_store.exists():
            raise ValueError("Chưa có storage_state của BAI.tools. Hãy login OAuth local và lưu session trước.")

...
DEFAULT_STORAGE_STATE_PATH = os.getenv(
    "AWESOME_INDIE_STORAGE_STATE_PATH",
    str(Path(__file__).resolve().parents[3] / ".playwright" / "awesome_indie_storage_state.json"),
)


class AwesomeIndieAuthStrategy(BaseAuthStrategy):
    ...
    async def ensure_authenticated(self, browser):
        if not self.session_store.exists():
            raise ValueError("Chưa có storage_state của Awesome Indie. Hãy login Google OAuth local và lưu session trước.")

...
DEFAULT_STORAGE_STATE_PATH = os.getenv(
    "ALTERNATIVE_STORAGE_STATE_PATH",
    str(Path(__file__).resolve().parents[3] / ".playwright" / "alternative_storage_state.json"),
)


class AlternativeAuthStrategy(BaseAuthStrategy):
    ...
    async def ensure_authenticated(self, browser):
        if not self.session_store.exists():
            raise ValueError("Chưa có storage_state của Alternative. Hãy login local bằng username/password trước.")

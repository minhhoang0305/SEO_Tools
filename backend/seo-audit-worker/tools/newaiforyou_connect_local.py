...
DEFAULT_OUTPUT = Path(__file__).resolve().parents[1] / ".playwright" / "newaiforyou_storage_state.json"
DEFAULT_PROFILE_DIR = Path(__file__).resolve().parents[1] / ".playwright" / "newaiforyou_chrome_profile"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Open New AI For You in real Chrome and save Google OAuth session.")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT, help="Path to save newaiforyou storage_state JSON.")
...
async def connect_newaiforyou(output_path: Path, profile_dir: Path, timeout_ms: int) -> None:
    async with async_playwright() as p:
        ...
        try:
            ...
            print("Chrome thật đã mở New AI For You.")
...
def main() -> None:
    args = parse_args()
    asyncio.run(connect_newaiforyou(args.output.expanduser(), args.profile_dir.expanduser(), args.timeout_ms))

    ...
    IMessagePublisher publisher)
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!string.IsNullOrWhiteSpace(request.TwitterHandle)) metadata["TwitterHandle"] = request.TwitterHandle;
        if (!string.IsNullOrWhiteSpace(request.TenWordsCategory)) metadata["TenWordsCategory"] = request.TenWordsCategory;
        if (!string.IsNullOrWhiteSpace(request.TenWordsNewsletter)) metadata["TenWordsNewsletter"] = request.TenWordsNewsletter;
        ...
        if (!string.IsNullOrWhiteSpace(request.PricingUrl)) metadata["PricingUrl"] = request.PricingUrl;
        if (!string.IsNullOrWhiteSpace(request.Type)) metadata["Type"] = request.Type;
        if (!string.IsNullOrWhiteSpace(request.Monetization)) metadata["Monetization"] = request.Monetization;
        if (!string.IsNullOrWhiteSpace(request.Status)) metadata["Status"] = request.Status;
        ...
        if (!string.IsNullOrWhiteSpace(request.Synonyms)) metadata["Synonyms"] = request.Synonyms;
